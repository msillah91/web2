...
if (isset($_POST['submit'])) {
			...
			$first=mysqli_real_escape_string($dbc,$_POST['name']);
			$last=mysqli_real_escape_string($dbc,$_POST['message']);
			$status=mysqli_real_escape_string($dbc,$_POST['status']);
 $q="insert into notifications (name,message,status) values ('$name','$message','$status')";
   			mysqli_query($dbc,$q);
...
<table border="1px" style="width: 800px; line-height: 30px;">
	...
	<thead>
		<tr><th colspan="11" style="padding-left: 27%;">VIEW NOTIFICATIONS</th></tr>
		<tr>
			<t>
			<th>Name</th>
			<th>Message</th>
